...
def crear_vale_resguardo(
    ...
    for item in data.detalles:
        if item.cantidad_entregada <= 0:
            raise HTTPException(status_code=400, detail="La cantidad entregada debe ser mayor a 0")
...
def registrar_devolucion(
    ...
    for dev in data.devoluciones:
        ...
        if not detalle:
            raise HTTPException(status_code=404, detail=f"Detalle {dev.detalle_id} no encontrado")

        if dev.cantidad_devuelta <= 0:
            raise HTTPException(status_code=400, detail="La devolución debe ser mayor a 0")
