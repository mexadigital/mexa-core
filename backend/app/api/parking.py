...
def ensure_tarifas(db: Session, organizacion_id: int):
    existentes = {
        t.tipo_vehiculo.lower(): t
        for t in db.query(ParkingTarifa)
        .filter(ParkingTarifa.organizacion_id == organizacion_id, ParkingTarifa.activa.is_(True))
        .all()
    }
    changed = False
    for tipo, precio in DEFAULT_TARIFAS.items():
        if tipo not in existentes:
            db.add(ParkingTarifa(
...
def tarifa_activa(db: Session, organizacion_id: int, tipo: str) -> ParkingTarifa:
    ...
    tarifa = db.query(ParkingTarifa).filter(
        ParkingTarifa.organizacion_id == organizacion_id,
        ParkingTarifa.tipo_vehiculo == tipo.lower(),
        ParkingTarifa.activa.is_(True),
    ).first()
    if not tarifa:
...
def turno_abierto(db: Session, organizacion_id: int) -> ParkingTurno | None:
    return db.query(ParkingTurno).filter(
        ParkingTurno.organizacion_id == organizacion_id,
        ParkingTurno.estado == "abierto",
    ).order_by(ParkingTurno.id.desc()).first()
...
def calc_importe(entrada: datetime, salida: datetime, tarifa: ParkingTarifa) -> tuple[int, Decimal]:
    ...
    # Primera hora se cobra completa. Hay 5 min de gracia antes de cada hora adicional.
    if minutos <= 60 + tolerancia:
        horas = 1
    else:
        horas = 1 + math.ceil((minutos - (60 + tolerancia)) / 60)
...
def movimiento_dict(m: ParkingMovimiento):
    return {
        ...
        "entrada_at": m.entrada_at,
        "salida_real_at": m.salida_real_at,
        "cierre_sistema_at": m.cierre_sistema_at,
        "minutos_cobrados": m.minutos_cobrados,
        "importe": float(m.importe or 0),
        "estado": m.estado,
        "nota": m.nota,
        "motivo_sin_estacionarse": m.motivo_sin_estacionarse,
...
class BanoIn(BaseModel):
    tipo_cliente: str = "externo"  # externo | estacionamiento
...
def dashboard(current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    activos = db.query(ParkingMovimiento).filter(
        ParkingMovimiento.organizacion_id == oid,
        ParkingMovimiento.estado == "dentro",
    ).order_by(ParkingMovimiento.entrada_at.asc()).all()
    historial = db.query(ParkingMovimiento).filter(
        ParkingMovimiento.organizacion_id == oid,
        ParkingMovimiento.estado != "dentro",
    ).order_by(ParkingMovimiento.id.desc()).limit(100).all()
    tarifas = db.query(ParkingTarifa).filter(
        ParkingTarifa.organizacion_id == oid,
        ParkingTarifa.activa.is_(True),
    ).order_by(ParkingTarifa.id.asc()).all()
    banos_turno = 0
    if turno:
        banos_turno = db.query(ParkingBano).filter(ParkingBano.turno_id == turno.id).count()
    return {
        ...
        "turno": None if not turno else {
            "id": turno.id,
            "operador": turno.nombre_operador,
            "apertura_at": turno.apertura_at,
            "fondo_inicial": float(turno.fondo_inicial or 0),
            "estado": turno.estado,
        },
        "tarifas": [{
            "id": t.id, "tipo": t.tipo_vehiculo, "precio_hora": float(t.precio_hora),
...
def abrir_turno(data: AbrirTurnoIn, current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    t = ParkingTurno(
        ...
        nombre_operador=current_user.nombre,
        fondo_inicial=Decimal(str(data.fondo_inicial or 0)),
        nota=data.nota,
        estado="abierto",
    )
    db.add(t)
...
def entrada(data: EntradaIn, current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    ocupados = db.query(ParkingMovimiento).filter(
        ParkingMovimiento.organizacion_id == oid,
        ParkingMovimiento.estado == "dentro",
    ).count()
    ...
    cliente = None
    if placa != "SIN PLACA":
        cliente = db.query(ParkingCliente).filter(
            ParkingCliente.organizacion_id == oid,
            ParkingCliente.placa == placa,
            ParkingCliente.activo.is_(True),
        ).first()
    if not cliente and (data.nombre_cliente or data.distinguido or placa != "SIN PLACA"):
        cliente = ParkingCliente(
            organizacion_id=oid,
            nombre=data.nombre_cliente,
            telefono=data.telefono,
            placa=None if placa == "SIN PLACA" else placa,
            tipo_vehiculo=tipo,
            distinguido=data.distinguido,
            activo=True,
        )
        db.add(cliente)
        db.flush()
...
def salida(movimiento_id: int, data: SalidaIn, current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    oid = org_id(current_user)
    m = db.query(ParkingMovimiento).filter(
        ParkingMovimiento.id == movimiento_id,
        ParkingMovimiento.organizacion_id == oid,
    ).first()
...
def registrar_bano(data: BanoIn, current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if data.movimiento_id:
        mov = db.query(ParkingMovimiento).filter(
            ParkingMovimiento.id == data.movimiento_id,
            ParkingMovimiento.organizacion_id == oid,
        ).first()
        if not mov:
...
def cerrar_turno(data: CerrarTurnoIn, current_user: Usuario = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not corte:
        corte = ParkingCorte(
            organizacion_id=oid,
            turno_id=turno.id,
            total_estacionamiento=total_est,
    ...
    return {
        "ok": True,
        "turno_id": turno.id,
        "total_estacionamiento": float(total_est),
        "total_banos": float(total_banos),
        "total": float(total),
        "efectivo_cierre": None if turno.efectivo_cierre is None else float(turno.efectivo_cierre),
    }
